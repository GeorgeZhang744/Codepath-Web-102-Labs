        ...
        {searchInput.length > 0
          ? filteredResults.map((coin) => {
              ...
              if (coinData.IsTrading && coinData.Algorithm !== "N/A" && coinData.ProofType !== "N/A") {
                return <CoinInfo key={coin} image={coinData.ImageUrl} name={coinData.FullName} symbol={coinData.Symbol} />;
              }
              return null;
            })
          : list &&
            Object.entries(list.Data)
              .filter(([_, coinData]) => coinData.IsTrading && coinData.Algorithm !== "N/A" && coinData.ProofType !== "N/A")
              .slice(0, 20)
              .map(([coin, coinData]) => (
                <CoinInfo key={coin} image={coinData.ImageUrl} name={coinData.FullName} symbol={coinData.Symbol} />
              ))}
      </ul>
    </div>
  );
